"""
Script that runs all sentence BERT embedding tests for Unequal Representation: Analyzing Intersectional Biases in Word Embeddings 
...
def find_keyword(sent, id_list):
    # Find the keyword, whose embedding will go into the reference model
    in_list = [wrd in id_list for wrd in sent.split()]
    try:
        return in_list.index(True)
    except:
        return None


def get_bert_embeds(bert_sents, grp1_id, grp2_id, attr_id):
    # Get BERT-base-uncased model and extract embeddings from it
    ...
    for sent in bert_sents:
        ...
        idx = find_keyword(sent, grp1_id)
        if idx is None:
            idx = find_keyword(sent, grp2_id)
        if idx is None:
            idx = find_keyword(sent, attr_id)
...
if __name__ == "__main__":
    ...
    # Read data
    sheets = pd.ExcelFile('Data/BERT_Sentence_Data.xlsx')
